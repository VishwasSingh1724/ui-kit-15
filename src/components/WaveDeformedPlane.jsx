...
const WaveScene = () => {
  ...
  const vertices = useMemo(() => new Float32Array(101 * 101 * 3), []);
  ...
  useFrame(() => {
    ...
    for (let i = 0; i < positions.length; i += 3) {
      ...
      const z = Math.floor(i / (101 * 3)) - 50;
      positions[i + 1] =
        Math.sin(MathUtils.degToRad(x + timeRef.current)) *
        Math.cos(MathUtils.degToRad(z + timeRef.current)) *
        2;
    }
  ...
  return (
    <mesh ref={planeRef} geometry={geometry} rotation={[-Math.PI / 2, 0, 0]}>
      <meshPhongMaterial color="lightblue" side={2} />
    </mesh>
  );
};
